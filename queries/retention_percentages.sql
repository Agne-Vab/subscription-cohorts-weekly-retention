...
SELECT
  sub.start_week_date,
  (SELECT COUNT(user_pseudo_id), FROM sub s WHERE sub.start_week_date = s.start_week_date) AS num_at_start,
  CONCAT(ROUND((w1.week_1/COUNT(user_pseudo_id))*100,2),"%") as week_1,
  CONCAT(ROUND((w2.week_2/COUNT(user_pseudo_id))*100,2),"%") as week_2,
  CONCAT(ROUND((w3.week_3/COUNT(user_pseudo_id))*100,2),"%") as week_3,
  CONCAT(ROUND((w4.week_4/COUNT(user_pseudo_id))*100,2),"%") as week_4,
  CONCAT(ROUND((w5.week_5/COUNT(user_pseudo_id))*100,2),"%") as week_5,
  CONCAT(ROUND((w6.week_6/COUNT(user_pseudo_id))*100,2),"%") as week_6
FROM
  sub
FULL JOIN
  (SELECT start_week_date, COUNT(user_pseudo_id) week_1
